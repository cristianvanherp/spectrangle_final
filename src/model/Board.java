...
public class Board {
	...
	/*@ pure */public String toString() {
		...
		Integer value;
		Character chrVertical, chrLeft, chrRight;
		for (Slot slot : this.slots) {
			value = null;
			chrVertical = null;
			chrLeft = null;
			...
			if (slot.getTile() != null) {
				...
				chrVertical = slot.getTile().getColorVertical();
				chrLeft = slot.getTile().getColorLeft();
				chrRight = slot.getTile().getColorRight();
			}

			values.add(value);
			vertical.add(chrVertical);
			left.add(chrLeft);
			right.add(chrRight);
		}
	...
	private void init() {
		...
		for (int i = 0; i < Board.LENGTH; i++) {
			...
			if ((rowOfIndex(i) + columnOfIndex(i)) % 2 == 1) {
				...
				slot.setOrientation(Orientation.UP);
			}
	...
	/*
	 *@ ensures (\exists Slot s; s.getSlots().contains(s) && s.getTile()!=null; \result==false);
	 * 
	 */
